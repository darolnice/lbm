class Index {
        ...
        $.ajax({
            ...
            success: function(response){
                ...
                if(response["response_code"] === 200 && response["res_id"] === 1012){
                    if (response['message']){
                        let link = 'http://localhost/projets/lebolma/assets/images/upload/'+response['message'][0]['profil_image'];
    ...
    post_asyn_fetch(url, context = null, data){
        fetch(url, {
            ...
            .then(dta => {
                 if (dta['res_id'] === 20){
                     let link_ = 'http://localhost/projets/lebolma/assets/images/upload/'+dta['message'];
    ...
    //Event source large
    wsEventsLarge(){
        let url = new URL('https://localhost:3000/.well-known/mercure/subscriptions');
        let user = new Index().getCookie('cookies_u_data');

        if(user !== null){
            // topic pour le chat de façon global
            url.searchParams.append('topic', 'http://localhost/projets/lebolma/');
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if(shp_u !== null){
        ...
        const es_shp_u = new EventSource(url);
    ...
    if(v_id !== null){
        ...
        m_url.searchParams.append('topic', 'http://localhost/projets/lebolma/');

        const es_v = new EventSource(url);
        es_vs.onmessage = (e)=>{
            if (dta.sujet === 'PPC'){
                alert(e.dta['message']);
            }
